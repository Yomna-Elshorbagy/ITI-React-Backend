...
export const getOrderDetails = catchAsyncError(async (req, res, next) => {
  ...
  const order = await Order.findOne({ _id: id, user: req.authUser._id }).populate(
    "products.productId"
  );
...
export const createOrderWithLocation = catchAsyncError(async (req, res, next) => {
  ...
  if (location) {
    ...
    if (!description && process.env.GOOGLE_MAPS_API_KEY) {
      try {
        ...
        description = data.results[0]?.formatted_address || "Unknown location";
      } catch (err) {
